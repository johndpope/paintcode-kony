...
function  containerWidgetExtendCreateHierarchy(widgetarray) {
	for (var i = 0; i < widgetarray.length; i++) {
		...
		widgetref.layoutConfig = {self: true, children: false};	// Resetting the layout config when removed from one container and adding to other
	}
};
...
function boxWidgetExtendRemove(widgetref) {
	if(widgetref){
		...
		if(widgetIndex != -1)
			$KU.onWidgetRemove(this, widgetIndex);
	}
};
