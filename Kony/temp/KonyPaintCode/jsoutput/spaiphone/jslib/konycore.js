kony.events = {
	...
	getEventHandler: function (eventObject) 
	{
        ...
        if(targetWidget.getAttribute('w-type') != "Calendar" && targetWidget.getAttribute('kwidgettype') != "Calendar" ){
			$KW.Calendar && $KW.Calendar.__dp.destroyCalendar(null, null, targetWidget);
		}
	    ...
	    if (targetWidget) 
		{
			...
			{ 
				...
				{
					// Fix for 32560: Added onclick support for Template HBox : renamed tpwidgettype as kwidgettype for googlemap widget in all places.
					...
					if(targetWidget && targetWidget.tagName == "CANVAS" && ($KU.isIE10 || $KU.isIE11) && konyWidget && konyWidget.getAttribute("kwidgettype") == "googlemap"){
					    return;
					}
			...
			if(targetWidgetType == 'RadioButtonGroup' || targetWidgetType == 'CheckBoxGroup' || targetWidgetType == 'ComboBox' || targetWidgetType == 'ListBox'){
				//Prevents click event after change event for default views of group widgets. The issue is when any non modal popup is opened in onselection event, the popup is getting dismissed in click event of that widget.
	...
	stopPropagation: function(eventObject){        
		...
		/*
		  when dismissing non model pop up we are stopping propagation "touchstart" event.
		  in that case TouchClick should not handle this case.
		*/
		...
		if(orientation != undefined && orientation == $KG["__orientation"] && event && event.type == 'resize'){	
			...
			kony.events.resizeTimeoutId = setTimeout(function(){
				if($KG.activeInput && !$KG["nativeScroll"] && winNewHeight > $KG['__viewportHeight']){
					...
					// Check for android chrome because of the suggestions bar that appears while typing. This causes applications login screen issues like dismissing the keyboard after tapping on the password field.
	...
	executeBoxEvent: function(wModel, rowdata, containerModel, isSection) {
        if(rowdata && containerModel) { //Then widgetModel is inside a container widget
	        ...
	        var pModel = wModel;
			while(pModel) {		
                // Fix for 32560: Added onclick support for Template HBox (top-level)			
				if ( pModel.onclick || (pModel.parent && formId == pModel.parent.id && pModel.parent.wType != 'HBox') || pModel.id == form.id) {
					if(pModel.onclick){
						...
						$KU.onEventHandler(pModel);
	                    return true;
					}
					return false;
				}
				pModel = pModel.parent || form;
	        }          
	        return false;
	    }
    },
		...
		handleBrowserBackEvent : function(){
		   // Reset the idle Time Out if the app has been set for it.
		    ...
		    if ((window.location.hash || window.location.hash ==="") && kony.events.browserback.currentHash && window.location.hash !== kony.events.browserback.currentHash){
		        ...
		        if(previousFormID === ""){
		            //previousFormID = formModel.id;
		            // Fix for 31946 : Browser back on intial app launch, need to navigate to previous URL.
		            window.history.go(-1);
		            return;
		        }
