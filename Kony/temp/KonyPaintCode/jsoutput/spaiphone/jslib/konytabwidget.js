...
$KW.TabPane =
{
	...
	repaint: function(tabPaneWidget, widgetModel){
		if(tabPaneWidget){
			if(widgetModel.viewtype === 'tabview' || widgetModel.viewtype == constants.TABPANE_VIEW_TYPE_PAGEVIEW){
				...
				$KW.TabPane.adjustFlexContainers(widgetModel);	
			}
	...
	generateCollapsibleViewTab : function(tabPaneModel,tabModel,index,context){
		...
		htmlString += "<div id = '" + tabId + "_Header' index=" + index + baseHtml + "kwidgettype='Tab' tabHeader='true' class='" + (isActive ? activeSkin : inActiveSkin) + "'" + (isActive ? " active='1'" : " active='0'") + ">";
	...
	generateAccordionViewTab :function(tabPaneModel,tabModel,index,context){
		...
		htmlString += "<div id = '" + tabId + "_Header' index=" + index + baseHtml + "kwidgettype='Tab' tabHeader='true' class='" + ((index == (tabPaneModel.activetab - IndexJL)) ? activeSkin + "' active='1'>" : inActiveSkin + "' active='0'>");
    ...
    expandTab: function(tabModel, tabWidget){
		...
		$KW.TabPane.adjustFlexContainers(tabModel.parent);
		var duration = tabModel.transDuration ? parseInt(tabModel.transDuration): $KW.TabPane.ANIMATION_DELAY;
		tabWidget.style[vendor + 'Transition'] = 'height ' + duration + 'ms ease';
        tabWidget.style.height = tabWidget.scrollHeight + 'px';
        setTimeout(function(){
	...
	/*
	* FTR 1060: setTabName API implementation for Tab Widget in SPA and DESKTOPWEB.
	* This API takes 2 arguments from user tabId and tabName. Updates tabModel and DOM with new tabname when valid tabId is provided.
	* Ignores updation of tabName if invalid tabId is provided.
	*/
	setTabName: function(tabPaneModel, tabId, tabName){ 
		var tabModel = tabPaneModel[tabId]; 
		if(tabModel){    
			tabModel.tabname = tabName; 	
			var tabId = tabPaneModel.pf + '_' + tabId;  
			if(tabPaneModel.viewtype == "tabview"){	
				var anchorElem = $KU.getElementById(tabId + '_A'); 
				if(anchorElem)
					anchorElem.childNodes[0].innerText = tabName; 	 
			}
    ...
    eventHandler: function(eventObject, target, srcElement, canExecute){
        ...
        if(tabPaneModel.viewtype === 'collapsibleview' && tabPaneModel.toggletabs === false ){
			...
			for (var i = 0; i < tabPaneModel.children.length; i++) {
                ...
                if (tabPaneModel.children[i] === tabID) {
					...
					$KW.TabPane.adjustFlexContainers(tabPaneModel);
                    tabPaneModel.activetab = i + IndexJL;
                    tabPaneModel.activetabs[IndexJL] = tabPaneModel.activetab;
                }
                else {
       ...
       /* var tabref = $KU.returnEventReference(tabPaneModel.onclick);
		...
		if((canExecute == true || typeof canExecute == 'undefined') && tabpaneonclickhandler){
			tabpaneonclickhandler.call(tabModel, tabModel,  tabPaneModel.activetab, isExpanded);
