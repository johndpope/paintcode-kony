...
<head>
    ...
    <% if(deviceCategory.equalsIgnoreCase("android")) {%>
        <meta name="viewport" id="metaViewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no"/>   
		<script> 
				if(window.devicePixelRatio >= 3 ){ 
					document.getElementById('metaViewport').setAttribute('content', 'width=440, maximum-scale=1, user-scalable=no');
				}
		</script>        
    ...
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        ...
        <%if(!deviceCategory.equalsIgnoreCase("android") && !deviceCategory.equalsIgnoreCase("nth") && !deviceCategory.equalsIgnoreCase("bb") && !deviceCategory.equalsIgnoreCase("iphone")){ %>
    ...
    <%}%>
    <%if(deviceCategory.equalsIgnoreCase("iphone")){ %>
		<link rel="stylesheet" type="text/css" href="<%=gpath%>/iphone<%=WEBUtil.getTheme()%>/konyspaiphone.css?ver=<%=version%>" media="screen and (device-width:320px)" /> 
		...
		<link rel="stylesheet" type="text/css" href="<%=gpath%>/iphone<%=WEBUtil.getTheme()%>/konyspaiphone375.css?ver=<%=version%>" media="screen and (device-width:375px)" />
		<link rel="stylesheet" type="text/css" href="<%=gpath%>/iphone<%=WEBUtil.getTheme()%>/konyspaiphone414.css?ver=<%=version%>" media="screen and (device-width:414px)" />
	<%}%>

    <%if(deviceCategory.equalsIgnoreCase("nth")){
        String nthcat = (String) session.getAttribute("devicecat");
        String userAgent = request.getHeader("user-agent");
        if(userAgent.indexOf("MSIE 10.0") != -1 || userAgent.indexOf("Trident") != -1)
        {
        	isIECSSLoaded = true;
        %>
        	<style id="theme" type="text/css" media="screen">@import "<%= String.format("%s/%s%s/kony%s%s10%s.css?ver=%s", gpath, deviceCategory, WEBUtil.getTheme(), deviceCategory, nthcat,320,version) %>";</style>
        <%
        }
...
<%}
      ...
      if(nthcat == null) 
         nthcat = "";
      if((deviceCategory.equalsIgnoreCase("bb") || deviceCategory.equalsIgnoreCase("nth") ) && !isIECSSLoaded)
    { 
    %>
        <script type="text/javascript" >
	...
	<%if(deviceCategory.equalsIgnoreCase("iphone")){ %>
		<script>
		  ...
		  var filename = "iphone/konyiphone" +category + ".css?ver=1.0.0";
		  var link = document.createElement("link");
		  link.setAttribute("rel", "stylesheet");
		  link.setAttribute("type", "text/css");
		  link.setAttribute("id", "theme");
		  ...
		  if(!category){
		   ...
		   link.setAttribute("type", "text/css");
		   link.setAttribute("id", "theme");
		   var retfilename =  "iphone/konyiphoneretina.css?ver=1.0.0";
