...
$KW.Scroller = {
	...
	initialize: function(scrollerNodes, type)
	{
		...
		else if($KG["nativeScroll"] && type == "Form")
		{
			...
			var formModel = window[curForm.id];
			if(formModel && $KU.isFlexContainer(formModel)){
		...
		for (var i = 0; i < scrollerNodes.length; i++) 
		{
			...
			if(type == 'Form' && $KU.isFlexContainer(widgetModel))
			...
			else if(swipeDirection == "both")	// both
			{
			   ...
			   //For backward compatability of scrollbox widgets, swipeDirection both and none will have vScroll & hScroll as true to support scrollToBegin and scrollToEnd events.
				options.vScroll = true;
				options.hScroll = true;
				options.scrollbox = true;
			}
			...
			if(type == "FlexScrollContainer" || (type == 'Form' && $KU.isFlexContainer(widgetModel))){
				if($KG["nativeScroll"]){
                ...
                if(widgetModel.onscrollstart){
                    ...
                    options.onScrollStart = function(){
                        ...
                        options.onscrollstart.call( options.widgetModel, options.widgetModel);
                    };
                }
                if(widgetModel.onscrolling){
                    ...
                    options.onScrollMove = function(){
                        var options = this.options;
                        options.widgetModel.contentoffset ={x: -this.x,y: -this.y};
                        options.onscrolling.call( options.widgetModel, options.widgetModel);
                    };
                }
                
                if(widgetModel.onscrolltouchreleased ){
                    options.onscrolltouchreleased = widgetModel.onscrolltouchreleased;                    
                    options.onScrollTouchReleased = function(){
                        var options = this.options;
                        options.widgetModel.contentoffset ={x: -this.x,y: -this.y};
                        options.onscrolltouchreleased.call( options.widgetModel, options.widgetModel);
                    };
                }
                /*
                ...
                */
                if(widgetModel.onscrollend){
                    ...
                    options.onScrollEnd = function(){
                        var options = this.options;
                        options.widgetModel.contentoffset ={x: -this.x,y: -this.y};
                        options.onscrollend.call( options.widgetModel, options.widgetModel);
                    };
                }
					...
					onScrollEnd = function(){
						...
						if(((this.options.vScroll && this.y >= this.minScrollY) || (this.options.hScroll && this.x >= this.minScrollX)) && this.options.pullInitFlag)
						{
							...
							if(typeof model  === "undefined")
                                return;
                            //height check added if the segment min height 0 then only fire on reachbegin event
							if(Math.abs(this.y) >= 20) {	
								kony.print("On Reach beginning");
								// Dispatch user event
						...
						if(((this.options.vScroll && this.y <= this.maxScrollY) || (this.options.hScroll && this.x <= this.maxScrollX)) && this.options.pushInitFlag)
						{
							...
							if(typeof model  === "undefined")
                                return;
                            //This check added incase of segment removeall/setData called
							if(Math.abs(this.y) >= 20) {
								kony.print("On Reach end");
								// Dispatch user event
            ...
            if((type == "ScrollBox" && $KU.isFlexContainer(formModel)) || $KU.isFlexContainer(formModel) || (type == "FlexScrollContainer" && (widgetModel.parent.wType === "FlexScrollContainer" || widgetModel.parent.wType == "FlexContainer"))){
                options.bubbleEvents = true;
            }
            
			// Pass actual scrollerDom Node rather than scroller ID
	...
	initializeNativeScroller: function(widgetModel, _scrollerDOMNode){
		...
		//Flex Form or flexScrollConatiner
		if($KU.isFlexContainer(widgetModel) && !widgetModel.enableScrolling){ 
    ...
    destroyFormScroller: function(formId){
        ...
        for (var i = 0; i < scrollerNodes.length; i++) 
		{
			$KW.Scroller.destroyInstance(scrollerNodes[i]);           
        }
    }, 
    ...
    destroyInstance : function(scrollerNode) {
    	if(!scrollerNode) {
    		console.warn("it should not happen");
    		return;
    	}
	...
	checkDOMChanges: function(_scrollerId, headerId, footerId)
	{
		...
		if(formModel.wType == 'Form' && $KU.isFlexContainer(formModel)){
			var _scrollerDOMNode = document.getElementById(_scrollerId);
	...
	setSLWHeight : function( formModel, _scrollerDOMNode, needRefresh)
	{
		...
		if(slwModel) 
		{
			...
			if(sLWidget){
				...
				// Fix for 32933: Segment pageview height calculation when segment is screenlevelwidget. Making page indicator visible in viewport.
			    ...
			    //Fix for 38225: screenlevelwidget height should be same as form scroller height
				...
				//sLWidget.style.height = (_scrollerDOMNode.offsetHeight - segPageViewFooterHeight) + 'px';
