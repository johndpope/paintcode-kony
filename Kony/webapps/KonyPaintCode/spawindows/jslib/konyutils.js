...
$KU = kony.utils = {
	...
	*/
    ...
    imgLoadHandler: function(event,img){
        ...
        //FW2209 : check for existance of image to avoid any asynchronous call updation fails
	...
	onImageLoadComplete: function(widgetModel, img){
		var parentModel, containerModel;
        ...
        if($KU.isFlexWidget(widgetModel)){
			widgetModel.layoutConfig = {self: true, children: false};
			if(img.getAttribute("kcontainerid")){
				var flexNode = $KU.getParentByAttributeValue(img, "kwidgettype", parentModel.wType);
				if(flexNode){
					//TODO: Can be further optimized to call only once after all the imgs are loaded in any seg API's
					var imgAutoGrowHeight = img.autoGrowHeight;
					if(containerModel.wType == 'Segment' && $KU.isFlexWidget(containerModel) && !containerModel.needScroller && parentModel.autogrowHeight && imgAutoGrowHeight){
						containerModel.layoutConfig = {self: true, children: false};
						$KW.FlexContainer.forceLayout(containerModel.parent);
					}	
					else	
						$KW.FlexContainer.forceLayout(parentModel, flexNode.parentNode);
				}	
			}
			else
        ...
        if(containerModel){
			var topLevelModel = window[containerModel.pf];
			if(topLevelModel && topLevelModel.wType == 'Popup'){
				 var popupConatiner = $KU.getElementById(topLevelModel.id + '_container');
			//Fix for 41568: when segment with image placed in popup - onImageLoad popup position calc issue. 
	...
	isIOSgt6: (/(iPad|iPhone);.*CPU.*OS ([7-9]|[1-9][0-9])_\d/i).test(navigator.userAgent), // to check if iOS version is greater than 6
	...
	{
		if($KU.isiPhone && (widgetModel.wType === "HBox" || widgetModel.wType === "Label"  || widgetModel.wType === "VBox" || widgetModel.wType == "Segment" || widgetModel.wType == "ScrollBox" || widgetModel.wType == "HStrip" || widgetModel.wType == "Slider" || widgetModel.wType == "RichText" || widgetModel.wType == "FlexContainer" || widgetModel.wType == "FlexScrollContainer"))
			return true;
		else 
			return false;
	},
	...
	*****/
	getAccessibilityValues : function(widgetModel, accessibilityObj, groupWidgetValue, rowIndex)
	{
		...
		// Append space other wise DOM generation is going to be like this <divrole='banner' .......</div>
		var accessAttr = " ";
		if(accessObj)
		{
	...
	setScrollHeight: function(boxModel, scrollBox){
		...
		if(boxModel.wType == "Segment" && boxModel.viewType == "pageview" && boxModel.needpageindicator){
			pageIndicator = document.getElementById(boxModel.pf + "_" + boxModel.id + "_footer");
		...
		if ((scrolldirection != "horizontal" && boxModel.wType == 'ScrollBox') || boxModel.needScroller) 
		{
			...
			if(scrollerInstance && boxModel.wType !== "ScrollBox"){
				scrollerInstance.scroller.style[vendor + 'Transform'] = ''
	...
	onEventHandler: function(wModel){
		...
		if(wModel){
			...
			if(isFlexWidget){
				topLevelModel = window[wModel.pf];
			}	
		}
		else	
	...
	updateGestureTempList: function(model, data){
		...
		if(data && data.length > IndexJL){
			tempList = model.rowtemplate ? [model.rowtemplate.id] : [];
			for(var i = IndexJL; i < data.length; i++) {
    ...
    /*
     ...
     has to set application behavior property. E.g. setapplicationbehaviors({escapeHtml:true})
	 Added application behavior skipEscapeHtml to provide backward compatability and skip default escaping also.
	 If skipEscapeHtml set to true returns original value from this method E.g. setapplicationbehaviors({skipEscapeHtml:true})
    */
    escapeHTMLSpecialEntities: function(text) {
        if (typeof text !== 'string') return text;
        //Fix for 35944: Providing backward compatability to skip default escaping of 5 characters. 
	...
	getValueAndUnit: function(wModel, value){		
		var flexModel = $KU.isFlexContainer(wModel) ? wModel : wModel.parent;
	...
	updateScrollFlag: function(wModel){
        ...
        wModel.needScroller = $KU.isFlexContainer(parentModel) || !!($KU.isPositiveInteger(height) && wModel.containerheightreference);
	},
	...
	setLayoutConfig: function(wModel, val, oldValue){
		if(val != oldValue)
			wModel.layoutConfig = {self: true, children: false};
		this.updateAutoGrowFlexConfig(wModel);
	},
	
	updateAutoGrowFlexConfig: function(wModel){
		var pModel = wModel.parent;
		if(pModel && pModel.wType == 'FlexContainer' && pModel.autogrowHeight){
			pModel.layoutConfig = {self: true, children: true};
			this.updateAutoGrowFlexConfig(pModel);	
		}
	},
	
	/* When new widgets are added, need to layout out all its nested children. 
	 * When a widget is removed, only subsequent siblings self layout is required. Need not layout its children.  
     */	 
	...
	layoutNewWidgets: function(boxModel, wArray){
		if(wArray && wArray.length > 0){
			this.updateWidgetsConfig(boxModel, wArray);
			var isFlexContainer = $KU.isFlexContainer(boxModel);
			if(isFlexContainer)
	...
	//Setting layoutConfig flag 'children' to flexcontainer is enough as layout engine takes care of its child widgets.
	...
	//Setting layoutConfig flag to next sibiling of removed widget is enough as layout engine takes care of its subsequent widgets.
	onWidgetRemove: function(boxModel, index){
		if($KU.isFlexContainer(boxModel) && boxModel.layouttype && boxModel.layouttype != kony.flex.FREE_FORM){ 
