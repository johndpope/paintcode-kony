$KW.Form = {
    ...
    initializeView: function(formId, isForm) {
		...
		var widgetsSupported = [$KW.Label, $KW.Calendar, $KW.HStrip, $KW.Segment, $KW.ScrollBox, $KW.TabPane, $KW.Line, $KW.Switch, $KW.TextField, $KW.DataGrid, $KW.Media, $KW.MenuContainer, $KW.ComboBox, $KW.Slider, $KW.Map, $KW.FlexContainer, $KW.FlexScrollContainer, $KW.Browser];
	...
	setFormDimensions: function(formModel){
		if($KU.isFlexContainer(formModel)){
	...
	initializeFlexContainers: function(formModel){
		...
		// CS_53425
		if($KG[formModel.id + '_scroller']) // CS_55531 & CS_55545
	...
	initializeFlexContainersInTemplate: function(formModel){
		if(formModel.headers && formModel.headers.length > 0)
			this.initializeBox(formModel, formModel.headers[0], $KG["needScroller"] ? formModel.id + "_header" : 'header_container');
		if(formModel.footers && formModel.footers.length > 0)
			this.initializeBox(formModel, formModel.footers[0], $KG["needScroller"] ? formModel.id + "_footer" : 'footer_container');		
	},
	
	initializeBox: function(formModel, boxModel, id){
		if(boxModel.wType == 'FlexContainer'){
			var flexNode = document.querySelector('#' + id + ' div[kwidgettype="FlexContainer"]');
	...
	initializeAllFlexContainers: function(containerModel){
		if($KU.isFlexContainer(containerModel)){
    ...
    addChildAt : function(formModel, widget, index)
	{
        if(($KG["__currentForm"] && formModel.id == $KG["__currentForm"].id) || formModel.wType == 'Popup')
        {
			...
			var htmlString = "";
			var isFlexContainer = $KU.isFlexContainer(formModel);
			if(isFlexContainer)
...
-            this.generateScroller(formModel);
		...
		return function(){
			...
			if(checkformmode(formModel)) 
            {
				...
				if (curForm && curForm.onhide && curForm.id != formModel.id) 
				{
					...
					curref && curref.call(curForm, curForm);
				}
				...
				{ 
					...
					$KW.Form.setMainContainerStyle(formModel);
	...
	endTransition: function(src, dest, currentForm, previousForm){
		...
		// adding gesture events after previous form animation completion events.
		// otherwise all these events will get attached to previous form. CS_37474
		$KW.Utils.initializeGestures(currentForm);
					//Added below to support gesture events for form widgets itself.
                    $KW.Utils.initializeGestures({"formmodel":currentForm});
    ...
    domChangeListener : function (mutations){
        mutations.forEach(function(mutation) {
            ...
            if(widgetModel && widgetModel.name){
                kony.print(mutation.type + ' -- '+ widgetModel.id);
