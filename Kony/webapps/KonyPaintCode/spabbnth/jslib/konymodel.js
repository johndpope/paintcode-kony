		...
		switch (propertyName) 
		{
            ...
            case "skin":
				...
				if (element)
				{
					...
					if($KU.isFlexWidget(widgetModel) && !(widgetModel.wType == 'FlexContainer' && !widgetModel.clipbounds)){
						element =  $KW.Utils.getWidgetNode(widgetModel).parentNode;
						if(!element) return;
						$KU.removeClassName(element, oldPropertyValue || defaultSkin);
						$KU.addClassName(element, propertyValue || defaultSkin);
					}					
				...
				/*
				* Update linespacing to label when skin property changed for Label widget since font-size mar vary.
            ...
            case "padding":	
				if($KU.isFlexContainer(widgetModel) || widgetModel.wType == 'Slider' || widgetModel.wType == 'Switch')
					return;
				if(widgetModel.wType == "Segment"){
					if(widgetModel.viewtype == constants.SEGUI_VIEW_TYPE_PAGEVIEW){
						...
						return;// Applying padding for Segment PAGEVIEW case only.			
					} 					
				}
				if(widgetModel.wType == "DataGrid")
				{
					...
					for(var i=0;i<rows.length;i++)
					{
						...
						{
							if(isFlexWidget)
								$KW.Utils.setPaddingByParent(widgetModel, rows[i].cells[j]);
							else
