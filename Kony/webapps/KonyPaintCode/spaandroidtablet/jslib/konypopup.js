$KW.Popup = {
	...
	render: function(popupModel){
		...
		var popupId = popupModel.id;
			...
			if($KG.nativeScroll && !(popupModel.ismodal) && !($KU.isWindowsPhone && $KU.isIE9)){
				// FW3703.
				// when pointer events are reigstered in windows phone , pointerdown evnet is not getting stopped.
				// in this case we have rely on click event for windows phone .
				// when we click on the body click events are not generating on some of the devices (e.g IOS)
				
				
				kony.events.addEventListener(document.body, $KU.isIDevice?$KW.touch.events.touchstart:'click',  $KW.Popup.dismissPopup ,false);
				// when user scrolls click event will not get generated. 
				// in this case we have to rely on window scrolling event.
				
				if(!$KU.isIDevice){
					kony.events.addEventListener(document.body, $KW.touch.events.touchstart,  $KW.Popup.dismissPopup ,false);
					kony.events.addEventListener(window, 'scroll',  $KW.Popup.onScrollDismissPopup ,false);
				}
			}
    ...
    show: function(popupModel){
		...
		/* #39286
		    if we open a popup when textfield or textarea is active ( through the code ),
			keyboard focus is not going away.Now if we tap or click anywhere on the screen
			that (first ) event is going to current active element ( i.e textfield or textarea ) . 
			so we need to explicitly remove the focus to active element before showing  popup.  
		*/
		...
		//Fix for 32688 : Gesture event not working for widgets in popup. Added below initialize gesture for popup widgets.
		$KW.Utils.initializeGestures(popupModel);
	...
	dismiss: function(model, flag){
        ...
        if(popupelemMain) 
		{
			//When popup is dismissed and if there is any change in form header height in the same closure, adjust form position to avoid the delay.
			...
			if(!model){
			    ...
			    model = window[id]; //Getting model for onhide event
			}
	...
	domremove: function(id)
	{
		...
		if (popupelem) 
		{
			...
			if(popupModel.enableScroll)
			{
				//For popup with containerheight, scroller timer will be created to adjust headers and footer. On popup dismiss this should be cleared.
...
//		popupelem.parentNode.style.visibility = "visible";
            ...
            if (elem && widget.wType != "Form") {
				...
				if(!$KG.nativeScroll){
					...
					if(scrollerInstance){
						...
						if(top){
							var parentModel = model.context.widget.parent;
							var parentElement = $KU.getNodeByModel(parentModel);
							if(parentElement.id == parentModel.id+"_"+parentModel.id){////this condition added to solve FW2625: setcontext anchor to footer issue
								top = Math.abs(scrollerInstance._offset(elem).top);//if footer
							}else{
    ...
    animationEnd: function(id, endTrans){
        ...
        return function(){
			...
			// Fix for 41796: Scrolling to bottom issue resolved when popup launched on safari browser.
	...
	onScrollDismissPopup: function(event){
		// Dismiss Popup only if page scrolled - incase of popup launched in form preshow(scroll delta is 0) 
		if(window.pageYOffset > 1) 
			$KW.Popup.dismissPopup(event); 
	},
	...
	dismissPopup: function(pf)
	{
		...
		if(pf && typeof pf == "object"){
			...
			//popup is dismissing when we scroll the popup 51815 
		...
		if(!pf || (window[pf] && window[pf].wType != "Popup"))
		{
			...
			if(popup)
			{
				...
				if(!$KU.isIDevice){
					kony.events.removeEventListener(document.body, $KW.touch.events.touchstart,  $KW.Popup.dismissPopup ,false);
					kony.events.removeEventListener(window, 'scroll',  $KW.Popup.onScrollDismissPopup ,false);
					}
							//CS_40657
							// on Native scroll if click on widget to dismiss non model popup , widget is generating click event.
							// this stops click event generation 
							// when we touch on form , pf or e is null 
							if(e)
