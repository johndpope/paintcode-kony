...
$KI.setappevents = function(eventobj){
   ...
   if($KG["appmode"] == constants.APPLICATION_MODE_HYBRID){
	...
	if((startform == null) || (startform.length == 0)) {
		...
		/*  
	            When page get loaded screen reader reads the page content from top to bottom if we don't move the mouse.
				This happens only for static web pages. But in case of AJAX based page loading , there is nothing in page to read.
				Adding aria attributes to body tag will solve this problem.
				 aria-busy="true",
				 aria-live="polite",
				 aria-relevant="additions",
				 aria-atomic="false"
				 
				 The above statements will make the screen reader to wait for the content to be loaded.
				 Once we know the content got loaded set busy attribute to false. This triggers the screen reader .
	*/
