...
$KW.Segment = {
    ...
    orientationHandler: function(formId, orientation) {
		...
		$KU.setScrollBoxesHeight(formId, "Segment");
		var segDivs = document.querySelectorAll("div[kwidgettype='Segment']");     
        if(segDivs){
            for(var i=0; i < segDivs.length; i++){
                 var segDiv = segDivs[i];
                 var segmentModel= $KU.getModelByNode(segDiv);
                 segmentModel.layoutConfig = {self:true,children:true};
                 segmentModel.parent.forceLayout();
            }
        }
        //$KW.touch.computeSnapWidths(formId, "Segment");
    },
    ...
    render: function(segmentModel, context) {
        ...
        if(!segmentModel.retainselection) {
            ...
            if(segmentModel.selectedindex  && segmentModel.viewtype == constants.SEGUI_VIEW_TYPE_TABLEVIEW) {
                setTimeout(function(){$KW.Segment.setFocus(segmentModel, $KU.getNodeByModel(segmentModel), segmentModel.selectedindex)}, 500);
            }
        }
		...
		if($KU.isFlexContainer(parentModel))
    ...
    generateSegment: function(segmentModel, context) {
		...
		//Fix for FW2671 : Applying widgetSkin only for segmentUI2
		var widgetSkin = (segmentModel.name == "kony.ui.SegmentedUI2") ? (segmentModel.widgetskin ? segmentModel.widgetskin: ""):(segmentModel.skin ? segmentModel.skin + "grp" : "");
    ...
    generateRows: function(model, context, data, liExists, rowIndex, secIndex) {
        ...
        if(color && model.separatorrequired) {
			...
			//Fix for FW2671 : adding background-clip - to support transparancy for border
            seperator = " style='background-clip:padding-box;border-bottom:" + border + ";" + (rowIndex > IndexJL ? "border-top:" + border : "") + "'";
        }
    ...
    /*
        Refreshing the segment scroller instance incase of removeall/setData API used on segment
    ...
    */
    ...
    modifyContent: function(model, data, action, rowIndex, secIndex) {
		...
		*/	
		...
		model.isvisible && this.adjustFlexContainers(model, action, secIndex, rowIndex, newSections);	
    },
	...
	adjustFlexContainers: function(model, action, secIndex, rowIndex, newSections){
		...
		switch (action) {
			...
			case "setsectionat":
				...
				for(var i = 0; i < newSections; i++){
					...
					var flexNode = section.querySelector('[secIndex="' + i + ',-1"] div[kwidgettype="FlexContainer"]');
					if(flexNode){
						var flexModel = $KU.getModelByNode(flexNode);
						$KW.FlexContainer.forceLayout(flexModel, flexNode.parentNode);
					}
	    ...
	    if($KU.isFlexWidget(model) && !model.needScroller && model.autogrowMode == kony.flex.AUTOGROW_HEIGHT && model.viewtype == constants.SEGUI_VIEW_TYPE_TABLEVIEW){
    ...
    adjustFlexContainersInRows: function(segmentModel, segNode, rows, secIndex, needHeight){
        if(rows && rows.length > 0){
            ...
            var rowNodes;
            if(segmentModel.hasSections)
                rowNodes = segNode.childNodes;
            else
                rowNodes = segNode.childNodes[0].childNodes;
            for(var i=0; i<len; i++){
                var rowNode = segmentModel.hasSections ? rowNodes[i+1] : rowNodes[i];
                //var selector = (typeof secIndex != "undefined") ? '[secIndex="' + secIndex + ',' + i + '"]' : '[index="' + i +'"]';
                if(rowNode){
                    $KW.Segment.adjustTopLevelFlexContainer(rowNode, needHeight);
                }
            }
            /*
            This below code is temporary fix to forcelayout of the first row in case of segment autogrow.
            because for the first row the parentFrame is not able to calcualte.
            */
            rowNode = segmentModel.hasSections ? rowNodes[1] : rowNodes[0];
            if(rowNode)  {
                $KW.Segment.adjustTopLevelFlexContainer(rowNode, needHeight);
            }
        }
    ...
    adjustTopLevelFlexContainer: function(rowNode, needHeight) {
    ...
    renderHBox : function(widgetModel, context, rowData, segmentModel, rowIndex) {
        ...
        htmlString += "<div class = 'ktable " + computedSkin + "'" + (topLevelBox ? $KW.Utils.getBaseHtml(widgetModel, context, null, rowData.accessibilityConfig, rowIndex) : $KW.Utils.getBaseHtml(widgetModel, context)) + "style='table-layout:fixed;" + style + "'><div class = 'krow " + layoutDirection + " kwt100'>";
