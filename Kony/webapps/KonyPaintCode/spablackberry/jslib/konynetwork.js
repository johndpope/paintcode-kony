$KI.net = {	
	...
	/*
	* New API as part of HttpRequest API.
	* FormData is used to create a form object and append params
	* e.g. var params= new kony.net.FormData(); params.append(key, value);
	*
	*/
	FormData : function(){
		var _formdata = "", that = this; 
		that.append = function(key, value){ 
			if( key == "undefined" || key == ""){
				throw new Error("FormData append Error: key cannot be empty");
			}
			if(_formdata == ""){
				_formdata = key +"="+value; // adding params as k1=v1
			}else{
				_formdata  += "&"+(key+"="+value); // appending to existing params  as k1=v1&k2=v2
			} 
		},
	...
	HttpRequest   : function (){
		...
		that.send               = function (data) {
			...
			if(data instanceof kony.net.FormData){
				data = data.toString(); // Calling toString() if it is Kony.net.FormData otherwise send the data directly to send method.
			}
		    _sendcount++;
			...
			ajaxobj.onComplete =  function(transport) {
				...
				if (transport.status == 200) {
					if (transport.responseText && transport.responseText.length > 0) {                        
						ajaxobj.addResponseText(transport); //removed callback check as its added while forward merge, which causing multiple call backs execution
					}
					// Empty response
					else {
					...
					//5XX - Server Error codes ref: http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html
				 	if (transport.status == 0 || (/5+/.test(transport.status.toString()) == true)) {
						//Loading script content using response text. For a entry(module.js) in manifest, when the module file is requested syncronoulsy then file is not fetched some times.  
						if (transport.responseText && transport.responseText.length > 0) {
							ajaxobj.addResponseText(transport);
							return;
						}
	...
	invokeserviceasync: function(posturl, postdata, Callback, info,method, timeout) {
		...
		return (function ajax() {
			...
			function invokecallback(callback, status, rettable, info) {
                //if(status==300 || status==400)  CS 53342
				...
				if (transport.status == 200) {
					if (transport.responseText && transport.responseText.length > 0) {
						...
						try{
							...
							/* 
							* Siteminder case: server returns invalid json(error html string)
							* Returning error response along with original response from server 
							* Fix for Daimler - Site Minder integration - Framework change - mail chain
							*/
							rettable = {
								"opstatus": "1",
								"errcode": "1013",
	...
	*/
    ...
    /* FTR1004: Kony.net.getCookies() API will take url as an input parameter and return all the cookies stored in 
    local cookie store on the client/device for that particular domain in the context of current application. */
    getCookies: function(url) {
    	kony.print("getCookies: " +url)
		if(url){
