...
kony.appinit = {  
    ...
    initializeWidgets: function(){
   	...
   	loadlibrarysync: function(){
        ...
        if (!($KI.themes)) {
            ...
            if (appInit.index > 0) 
                kony.print("File loaded in sync: " + appInit.syncFiles[appInit.index - 1] + "  number: " + appInit.index);
    ...
    ondone: function(){
        var appInit = kony.appinit;        
        kony.print("File loaded in async: " + appInit.asyncViewFiles[appInit.index] + "  number: " + appInit.index);
        appInit.index++;
	...
	verifyhref : function(skiploadappjs) {		
		...
		if (searchstr == "") {	
			...
			if (hash != "") {
				...
				if (lform == hash) {
					kony.print("verifyhref:Last Form Stored: " + lform);															
				}
				else {
					kony.print("verifyhref:Last Form Stored: " + lform + " Hash: " + hash);
				}
				if(skiploadappjs)
	...
	disablesplash: function () {		
		...
		if(!(kony.appinit.isIE8 || kony.appinit.isIE9))
			$KG["animStyleSheet"] = $KU.createStyleSheet("animSheet"); // creating animation style sheet.
