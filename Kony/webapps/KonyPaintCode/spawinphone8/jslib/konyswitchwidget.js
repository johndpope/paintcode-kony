...
$KW.Switch = {
    ...
    render: function(switchModel, context){
		...
		if(kony.appinit.isIE9 || kony.appinit.isIE8){
			var htmlString = "<div" + $KW.Utils.getBaseHtml(switchModel, context) + "class='ktable kwt100 switch " + computedSkin + "' style='" + margin + ";position:relative'>" + 
			...
			if($KU.isIOSgt6){
				...
				var offStyle    = [
					...
					"-webkit-border-radius:15.5px",
					""].join(";");
					var htmlString = "<div role='option' aria-selected='" + [ 'true' , 'false' ][switchModel.selectedIndex] + "' " + substituteAccessAttr + $KW.Utils.getBaseHtml(switchModel, context) + "class='switch " + computedSkin + " " + (switchModel.skin ? switchModel.skin + "on" : "") + "' style='" + margin + switchStyle + (!switchModel.skin ? ";background: #4FD065" : "")+ "'>" + 
			...
			var htmlString = "<div role='option' aria-selected='" + [ 'true' , 'false' ][switchModel.selectedIndex] + "' " + substituteAccessAttr + $KW.Utils.getBaseHtml(switchModel, context) + "class='kwt100 switch " + computedSkin + "' style='"+ margin +"'>" + 
	...
	adjustWidth: function(switchModel, switchNode, attachEvent){
		if(!(kony.appinit.isIE9 || kony.appinit.isIE8)){
			if($KU.isIOSgt6) {
				...
				switchModel.sWidth = switchModel.cWidth + (switchNode.clientWidth % 2) + 6;
				switchNode.children[0].style.width = (switchModel.sWidth - 2) + "px"; //ON div 1px border was showing up so subtracting that
	...
	toggleSwitch: function(model){		
		...
		{
			this.applyTrans(switchDiv, $KU.isIOSgt6 && "all 0.4s ease-in-out" || "all 0.125s ease-in-out");
		 	if (model.selectedindex == IndexJL) {
		 		...
		 		if(kony.appinit.isIE9 || kony.appinit.isIE8){
					...
					if($KU.isIOSgt6) {
						...
						switchDiv.children[0].style[vendor + 'Transform'] = translateOpen + (-(model.cWidth + 8)) + "px,0" + translateClose;
						switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + (-model.cWidth + 2) + "px,0" + translateClose;
		 		...
		 		if(kony.appinit.isIE9 || kony.appinit.isIE8){
					...
					if($KU.isIOSgt6){
						...
						switchDiv.children[2].style[vendor + 'Transform'] = translateOpen + "0,0" + translateClose;
					}
		 		}
		 	}
			...
			$KW.Switch.addA11YAttribute( model, switchDiv);                           // add left || right-side-text
		 	if(modelref){
				modelref.call(model, model);
				$KU.onEventHandler(model);
			}	
		}		
	},
	...
	adjustSwitch: function(model, switchDiv, trans){        
        ...
        if (model.selectedindex == IndexJL) {
            switchDiv.children[0].style[vendor + 'Transform'] = translateOpen + "0,0" + translateClose;
			if($KU.isIOSgt6){
				switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + "21.5px,0" + translateClose;
				switchDiv.children[2].style[vendor + 'Transform'] = "scale(0.01)";
			}
			else{
				switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + "0,0" + translateClose;
            ...
            switchDiv.children[0].style[vendor + 'Transform'] = translateOpen + (-(model.cWidth + 6)) + "px,0" + translateClose;
			if($KU.isIOSgt6){
				switchDiv.children[1].style[vendor + 'Transform'] = translateOpen + 1.5 + "px,0" + translateClose;
				switchDiv.children[2].style[vendor + 'Transform'] = "scale(1)";
			}
