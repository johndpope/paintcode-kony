...
$KW.Map =
{
    ...
    initializeView: function(formId){
		...
		//Fix for 32444 - Map ScreenLevelWidget : Do not calculate height if map is SLW.  
    ...
    render: function(mapModel, context){
		...
		//therefore, appended to <td> instead
            ...
            htmlString = "<div kwidgettype='googlemap' kformname= '" + mapModel.pf + "' id='" + mapModel.pf + '_' + mapModel.id + "' name='map_canvas'" + isPolygonview + " style='height:500px;" + $KW.skins.getMarginSkin(mapModel, context) + $KW.skins.getPaddingSkin(mapModel) +"' class='" + computedSkin +
	...
	cachePinSizes:function(urlt){	
		/* Added this function of cachePinSizes to solve ticket41618
		-map call out images when loaded from different networks, loading time varies depending on the network bandwidth and number of request to host server. The size of last pinImage was only considered .
		To resolve this issue creating a mapping which stores the pin image and its size at img.onload function and later retrieving the size of the image from the mapping at the time of mapcallout.
		Note:Assuming that all the pin image sizes are updated before the user clicks on the marker. If this does not occur in some network then we need to handle this scenario in future*/
		if(typeof $KW.Map.pinImageSize === 'undefined')	
			...
			if(!$KW.Map.pinImageSize[mapKey]){
				img.onload = function(){
					$KW.Map.pinImageSize[mapKey] = this.naturalHeight;										
				}
			}										
	},	
        ...
        if (mapModel && mapModel.mapsrc === "static" && ($KU.isBlackBerryNTH || ($KU.isWindowsPhone && $KU.isIE9))) {
            ...
            if (mapCanvasElement) {
                try
				{
                    ...
                    window.console && kony.print("google : map script has not loaded yet");
                    return;
                }
                 ...
                 */
                        ...
                        if (mapview == null  && mapModel.mode!="5") {
                            ...
                            for (var i = IndexJL; i < (mapdata.length); i++) {
								...
								this.cachePinSizes(urlt); //added this to solve ticket 41618 mapcallout not positioned properly in cloud.
                                ...
                                if(mapInfoTemplateData){
									...
									// Forcelayout the container before use of it when the template is flex
                                ...
                                var marker1 = new google.maps.Marker({
                                    ...
                                    data:data,
                                    calloutPostion: (mapModel.calloutPostion || "top"),
                                    ...
                                    if(this.showcallout||this.showcallout==undefined||this.forceShowcallout)
                                    {
                                        if(this.template){
                                            ...
                                            if(this.calloutPostion != "bottom"){
                                                ...
                                                //var img = new Image();
                                                //img.src = this.icon;												
                                                //infobox_top = -(img.naturalHeight?img.naturalHeight:30);
												var mapKey = (this.icon).slice(this.icon.lastIndexOf("/")+1); 
												infobox_top = -($KW.Map.pinImageSize[mapKey]?$KW.Map.pinImageSize[mapKey]:30);
                                            }
                                            ...
                                            this.data && $KW.Utils.updateLayoutData(mapModel,this.template,this.data); // Fix for 40445 : Mapcallout template box click - model update to get correct pin data 
	...
	mapClickEventHandler: function(event) { 
		 ...
		 if(mapModel.onclick) {
			...
			//expecting a JS object not array.
    ...
    navigateToLocation: function(mapWidget, locationData, showcallout, dropPin){
        ...
        if(showcallout) {
            //Fix for 32560: added custom callout template implementation for NavigationToLocation
			...
			if(coData){
				...
				if(mapInfoTemplateData.containerWeight<1 || mapInfoTemplateData.containerWeight>100)
					mapInfoTemplateData.containerWeight = 80;
				var infobox_width = mapInfoTemplateData.containerWeight && (( docwidth * mapInfoTemplateData.containerWeight * 0.01 ) + "px") || "280px"; 	 
				var infobox_left  = parseInt(infobox_width, 10)/-2;  
				infobox.alignBottom_ = true;
				//var img = new Image();
				//img.src = urlt;												
				//var infobox_top = -(img.naturalHeight?img.naturalHeight:30); 
				var mapKey = (this.icon).slice(this.icon.lastIndexOf("/")+1); 
				infobox_top = -($KW.Map.pinImageSize[mapKey]?$KW.Map.pinImageSize[mapKey]:30);
				infobox.content_ = contentString, //String of the box which we recieved
				infobox.disableAutoPan_ = false,
	...
	// Fix for 32560: Adding infobox function (used in navigateToLocation and rendering)
