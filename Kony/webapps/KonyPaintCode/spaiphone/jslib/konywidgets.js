...
$KW.Widget = {
	...
	setvisibility: function(widgetModel, value){    
        ...
        if (widgetModel) 
		{
			if(widgetModel.parent && widgetModel.parent.wType == "TabPane") //do not update visibility for tab
				return;
...
/* 
                  CS_42033
				  if we set master data map when widget visibility is false ,
				  then it will not effect. 
				  scenario #1:
...
*/
	...
	/** When visibility is changed for a widget. 
	 * 
	 */	
	onVisibilityChange: function(wModel, value){
		/*var parentModel = wModel.parent;
		if(parentModel){
			var isFlexContainer = $KU.isFlexContainer(parentModel);
			if(isFlexContainer){
				var layoutType = parentModel.layouttype;
				if(layoutType == kony.flex.FREE_FORM && !value)
					return;
				$KU.updateLayoutConfig(wModel);
				value && wModel.ownchildrenref && $KU.updateWidgetsConfig(wModel, wModel.ownchildrenref);	
		...
		else if(wModel.ownchildrenref){ //Hbox, Vbox or other containers	
			$KU.updateWidgetsConfig(wModel, wModel.ownchildrenref);
			$KW.Form.initializeFlexContainersInBox(wModel);
		}
		else if(wModel.wType == 'Segment' && value){
		...
		$KU.updateLayoutConfig(wModel);
		$KU.updateAutoGrowFlexConfig(wModel);
		value && wModel.ownchildrenref && $KU.updateWidgetsConfig(wModel, wModel.ownchildrenref);
	},
	...
	setgesturerecognizer: function(widgetModel, gestureType, gestureObj, callback)
	{
        ...
        //Sumanth: Fixed issue with tap not firing when double tap is registered.
	...
	getPreferredWidth: function(widgetModel, element, maxWidth, updateUI){
		...
		/*if(!(widgetModel.wType == 'Switch' && $KU.isIOS7))
		...
		if(!defaultWidth){ //content driven widgets
			...
			element.style.whiteSpace = (wrapper.offsetWidth >= maxWidth) ? 'pre-wrap' : 'nowrap';
		}
	...
	getPreferredHeight: function(widgetModel, element, maxHeight, updateUI){
		...
		if(widgetModel.wType == "FlexContainer" && widgetModel.autogrowMode == kony.flex.AUTOGROW_HEIGHT && widgetModel.canMeasureChildrenHeight){
			//When we have nested autogrow flexcontainers, returing node height as inner flex containers height is already computed during top level container.
 ...
 */
    ...
    getPaddingSkin: function(widgetModel){
		...
		if(widgetModel.wType != "DataGrid" &&( $KU.isFlexContainer(widgetModel) || $KU.isFlexWidget(widgetModel)))
    		return "";
...
$KW.Utils = {
				...
				switch(wType) {        
					...
					case "FlexScrollContainer":
						...
						if(scrollBox){
							...
							//$KW.FlexScro=llContainer.forceLayout(widgetModel, $KU.getNodeByModel(widgetModel));
						}
						break;	
					...
					case "Browser":
						...
						break
					/*
					* Update linespacing to label when dynamically Label widget added to UI.
	...
	getContainerModelById: function(node, containerId){
		...
		return cur ? $KU.getModelByNode(cur) : '';
	},	
    ...
    applyBlockUISkin:function(model)
    {
		...
		// it was called on orientationEvent
		var cachedEL = undefined
		var imgWidth=undefined;
		var imgHeight=undefined;
		
	// called by Form on behalf of device orientation change. use cached data.
	if(!model){
	        if(!$KG.blockedSkinModel)
				return;			
				
				cachedEL = $KU.getElementById('blurDiv');
				if(!cachedEL){ // it should exists definitely. Otherwise we are missing other use cases.
	...
	// only create when it is not in DOM
	var blockUIDivElement = cachedEL || document.createElement("div");
	$KG.blockedSkinModel = model; // cache the model     
        ...
        // append only when we create it or leave as it is .
		!cachedEL && document.body.appendChild(blockUIDivElement);
		
        // why redundant call? use cached one or created one.
		...
		// use cached height and width not the increased width and height.
        
		var heightofimage = imgHeight || $KU.getStyle(el,"height");
        var widthofimage = imgWidth || $KU.getStyle(el,"width");
		
		// cache the original width and height for future reference.
		if(!imgHeight)
		...
		*/
        ...
        if (progressindicatorposition == "Sans-Serif") //For Centering the BlockUI image on top... top center
        {
	...
	// invalidate the model ,height & width .
	...
	*/
		...
		if(formGestures)
		{
			for(var k in formGestures)
			{
				...
				if(gesture){
                    for(var i in gesture ){
                       var gEObj = gesture[i];
                       $KW.Utils.removegesture(formModel, gEObj.gestureType,false,
                                                    gEObj.gestureIdentifier,gEObj.gestureObj);
					   new $KW.touch.gesture(formModel, gEObj);
                    }
				}
			}
		}
	...
	updateModelWithGesture: function(widgetModel, gestureType, gestureObj, callback)
	{
        ...
        var gestureEventObject = {gestureObj:gestureObj, callback:callback, gestureType:gestureType}
             ...
             for(var k in gestureList){  
                ...
                gestureList[k]["instance"].removeGesture(gestureType, updateModel);
				        ...
				        if(console && console.error)
							console.error(' remove Gesture ' + e);
				 }
            }
            if(updateModel)
                widgetModel.gestures[_gestureType]=[];
	...
	initializeScrollEvents : function(scrollNodes, wModel){		
		for (var i = 0; i < scrollNodes.length; i++) 
		{
			var scrollNode = scrollNodes[i];
			wModel = wModel || $KU.getModelByNode(scrollNode);
			if(!wModel.onScrollStart && !wModel.onScrolling && !wModel.onScrollEnd){
		    	return;
		    }
	...
	*/
	...
	/*
	* In case of Device Keypad prev/next button, scrollTop changed by browser hence resetting 
	* the scrollTop to zero and making scroller get position at that place.
	* Invoked from input Elements (TextField, TextArea, ListBox )
	*/
	resetScrollTopToContainer: function(eventObject){
		if($KG.nativeScroll){ // If native scroll is enabled, do not reset the scrollTop
			return;
		}
	...
	bringToView: function(el, isOnKeyboardPopup){
		if(!el){
			return; // element not defined, then do nothing
		}
		
		var scrollSelector = $KU.isFlexWidget($KU.getModelByNode(el))?'FlexScrollContainer':'ScrollBox';
                                                                                                
		//$KU.isKeypadOpened = false;
		...
		while(true){
			...
			var scrollerInstance = scrollBox ? $KG[scrollBox.id + "_scroller"] : ($KG["__currentForm"] ? $KG[$KG["__currentForm"].id + "_scroller"] : "");
				...
				if(parentEL.scrollTop > 0){
					 kony.print('scrolltop value found');
	...
	setFlexContainerStyle: function(flexModel, flexNode){
		...
		else if(!parentModel){ //header or footer
		...
		if(parentModel && parentModel.wType == 'TabPane' && $KU.isFlexWidget(parentModel)){
		...
		if(!$KU.isValidCSSLength(height)){
			//When autogrow flexcontainer used as rowTemplate / SecTemplate / header / footer
			if(flexModel.wType == 'FlexContainer' && flexModel.autogrowMode == kony.flex.AUTOGROW_HEIGHT){
				...
				height = $KW.Utils.getDefaultHeight(flexModel, false);
		}
		...
		if(height){
			...
			unit = obj.unit;
			if(unit == kony.flex.PERCENTAGE){
				if(containerModel && containerModel.wType == 'Segment' && !containerModel.needScroller && containerModel.autogrowMode == kony.flex.AUTOGROW_HEIGHT){
					value = 100;
				}
				else if(parentModel && $KU.isFlexContainer(parentModel))
			...
			if(containerModel && containerModel.wType == 'Segment' && containerModel.viewtype == constants.SEGUI_VIEW_TYPE_PAGEVIEW && containerModel.needpageindicator){
				...
				if(containerModel.viewtype === 'tabview' ||containerModel.viewtype === 'pageview'  ){
					var header = $KU.getElementById(containerModel.pf + "_" + containerModel.id + "_Header");
					headerHeight = header.offsetHeight;					
				}
				else{
					var header = $KU.getElementById(flexModel.pf + "_" + flexModel.id + "_Header");
					headerHeight = header.offsetHeight;	
				}
				node.style.height = (value - headerHeight) + 'px';
			}	
			else	
    ...
    getWidth: function(wModel, layoutModel, wNode, flexNode, updateUI){
		...
		if(!isWidthSet){
			...
			var calWidth = (implicitWidth != undefined) ? implicitWidth : $KW.Widget.getPreferredWidth(wModel, wNode.childNodes[0], maxWidth, updateUI);
	...
	getHeight: function(wModel, layoutModel, wNode, flexNode, updateUI){
		...
		if(!isHeightSet){
			...
			var calHeight = (typeof implicitHeight != "undefined") ? implicitHeight : $KW.Widget.getPreferredHeight(wModel, wNode.childNodes[0], maxHeight, updateUI);
		...
		if(updateUI	== undefined){
			...
			if(wModel.wType == "Segment" && wModel.viewtype == constants.SEGUI_VIEW_TYPE_TABLEVIEW && !wModel.needScroller && (isHeightSet || typeof implicitHeight != "undefined" || maxHeightObj)){
	...
	getChildHeight: function(wModel, wNode){
		if(wModel.autogrowMode == kony.flex.AUTOGROW_HEIGHT){
			if(wModel.wType == "FlexContainer"){
				...
				wNode.parentNode.style.height = 'auto';
				return this.getComputedPreferredHeight(wModel, wNode, $KW.Segment.getAutoGrowSegmentHeight(wModel, wNode));
			}
		}
		else{
			//Sumanth - 44998: Apply padding before computing the height for autogrow. 	
			$KW.Utils.setPaddingByParent(wModel, wNode);
	...
	getComputedPreferredHeight: function(wModel, wNode, computedheight){
		...
		if(typeof computedheight == "undefined"){
			computedheight = $KW.Widget.getPreferredHeight(wModel, wNode, maxHeight);
			if(wModel.wType == 'Image' && wModel.imagescalemode == constants.IMAGE_SCALE_MODE_MAINTAIN_ASPECT_RATIO){
				var dimensions = {width: wNode.offsetWidth + 'px', height:wNode.offsetHeight + 'px'};
				dimensions = $KU.setImgAspectRatio(wModel, wNode.childNodes[0], dimensions, false);
	...
	getWidgetDimensions: function(wModel, wNode){
		...
		if(parentModel && parentModel.frame && typeof parentModel.frame.x != 'undefined'){
			...
			var widthObj = {value: parseFloat(width), unit: $KU.getUnit(width)};
			width = $KU.getValueByParentFrame(wModel, widthObj, 'x');
			var height = wNode.style.height;
			var heightObj = {value: parseFloat(height), unit: $KU.getUnit(height)};
			height = $KU.getValueByParentFrame(wModel, heightObj, 'y');
		}
		else{
	...
	setWidgetPosition: function(wModel, frame, wNode){
		...
		for(var pos in frame){
			wNode.style[pos] = this.getPercentValue(wModel, frame[pos], pos);
		}
		if(!wNode.style.left)
			wNode.style.left = '0%';
		if(!wNode.style.top)
	...
	getWidgetNode: function(model, parentNode){
		...
		if(widgetTabId)
			widgetID = model.pf + "_" + widgetTabId + "_" + model.id
		else
			widgetID = (model.pf ? model.pf + "_" : "") + model.id;
	...
	setPaddingByParent: function(wModel, wNode){
		if($KU.isFlexContainer(wModel) || wModel.wType == 'Slider' || wModel.wType == 'Switch')
			return;
		var parent = wModel.parent;
